...
int16_t clamp(int16_t input, int16_t low, int16_t high) {
	if (input > high) {
		return high;
	}
	if (input < low) {
...
void PI_update(void) {
     ...
     */
		 ...
		 int16_t output = 0;
		 int16_t target_speed = target_rpm * 2;
    ...
    /// TODO: Calculate integral portion of PI controller, write to "error_integral" variable
		...
		error_integral = clamp(error_integral, 0, 500);
		
		output += error_integral * Ki;
